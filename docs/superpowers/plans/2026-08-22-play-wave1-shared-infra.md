...
- **Play-zone accent vocabulary (NEW this wave — read carefully, this precedent governs 5 more waves):** tokens `--play-400: #fcd34d`, `--play-500: #f59e0b`, `--play-600: #d97706` — approximated in Tailwind class form via `amber-300`/`amber-500`/`amber-600` (this project's convention throughout the redesign; note Tailwind v4's amber ramp is defined in oklch, so these classes render *close to but not byte-identical to* the token hexes — same category of approximation as Hub's `--hub-500`≠`indigo-500` naming drift, established during Foundation). **`amber-400` is reserved for `--mb-prestige`/`--accent-gold` (the cross-zone PRO/prestige semantic) — never use it for Play-zone decorative chrome.** This is a semantic reservation, not a hex-identity claim (verified during Play Wave 2's final review: under Tailwind v4, `amber-400` and `--accent-gold` are close golds but not byte-identical) — the rule holds regardless: `amber-400` is the PRO/prestige rung, always use `amber-300`/`amber-500`/`amber-600` for Play-zone brand chrome instead.
