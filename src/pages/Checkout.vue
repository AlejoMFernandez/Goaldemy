    ...
    <div v-else-if="plan" class="grid grid-cols-1 md:grid-cols-[1fr_360px] gap-8 items-start">
      ...
      <div class="order-1 md:order-2 md:sticky md:top-24 rounded-2xl border border-white/15 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
        ...
        <div class="mt-4 flex items-start gap-2 text-xs text-slate-500">
          ...
          <span>Pagos seguros con <strong class="text-slate-400">Mercado Pago</strong>. Podés pagar con tarjeta, débito, dinero en cuenta o efectivo, y cancelás cuando quieras.</span>
        </div>
      </div>
    </div>
  </div>
